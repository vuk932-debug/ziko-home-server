...
router.put('/properties/:id/view', async (req: any, res: any) => {
  await pushRecentlyViewed(req.user.id, req.params.id);
  res.status(200).send();
});

router.post('/properties/:id/save', roleMiddleware('Customer'), handleSaveProperty);
router.get('/saved', roleMiddleware('Customer'), getSavedProperties);
